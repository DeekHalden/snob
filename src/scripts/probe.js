...
function fade(startValue, endValue, computedNow, element, valueNow, direction) {
    ...
    if (direction > 0) {
        if (computedNow >= endValue - 20 && computedNow <= endValue) {
            var el = $(element).children('video').fadeIn();
            $(el)[0].play();
            $(element).children('.slide__text').addClass('active--left');
            $(element).find('.slide__paragraph').addClass('slide__paragraph--active');
            $(element).find('.slide__head-description').fadeIn(2000 /*'slide__head-description--active'*/ );
        } else if (computedNow >= endValue / 1.5) {
            $(element).parent().find('.' + firstClassName + '--' + (secondClassName.slice(-1) - 1)).children('.slide__text').removeClass('active--left').addClass('active--right');
        } else if (computedNow >= endValue / 2) {
            // $(element).find('.slide__paragraph').addClass('slide__paragraph--hidden');
            $('video').fadeOut();
        } else if (computedNow >= endValue / 2.5) {
            $(element).parent().find('.' + firstClassName + '--' + (secondClassName.slice(-1) - 1)).children('.slide__head-description').fadeOut( /*'slide__head-description--active'*/ 100);
        }
    } else {
        if (computedNow >= endValue - 20 && computedNow <= endValue) {
            $(element).children('.slide__text').removeClass('active--right').removeClass('active--left');
            $('video').fadeOut();
            // $(element).parent().find('.' + firstClassName + '--' + (secondClassName.slice(-1) + 1)).children('.slide__text').removeClass('active--right').addClass('active--left');
            // $(element).children('.slide__text').removeClass('active--right');
        } else if (computedNow <= endValue / 2.5) {
            console.log(12)
            $(element).parent().find('.' + firstClassName + '--' + (secondClassName.slice(-1) - 1)).children('.slide__head-description').fadeIn( /*'slide__head-description--active'*/ 2000);
            ...
            $(element).parent().find('.' + firstClassName + '--' + (secondClassName.slice(-1) - 1)).children('.slide__text').removeClass('active--right').addClass('active--left');
        } else if (computedNow <= endValue / .5) {
            $('.slide__head-description').fadeOut();
        }
    }
}
...
$(document).ready(function() {
    ...
    $("#controls").ionRangeSlider({
        ...
        onChange: function(data) {
            ...
            $('.first-part').fadeOut();
            let firstBreakpoint = $('.range-slider__anchor')[1].offsetLeft - 110;
            let secondBreakpoint = $('.range-slider__anchor')[2].offsetLeft - 110;
            let thirdBreakpoint = $('.range-slider__anchor')[3].offsetLeft - 110;
